...
def open_chrome(permissions: dict[str, bool]) -> str:
    if not allowed(permissions, "open_apps"):
        return "I do not have permission to open apps yet."
    chrome_paths = [
...
def open_vscode(permissions: dict[str, bool]) -> str:
    if not allowed(permissions, "open_apps"):
        return "I do not have permission to open apps yet."
    try:
...
def open_youtube(permissions: dict[str, bool]) -> str:
    if not allowed(permissions, "open_websites"):
        return "I do not have permission to open websites yet."
    webbrowser.open("https://www.youtube.com")
    return "Opening YouTube."


def search_google(query: str, permissions: dict[str, bool]) -> str:
    if not allowed(permissions, "open_websites"):
        return "I do not have permission to open websites yet."
    cleaned = query.strip()
    if not cleaned:
...
def create_folder(name: str, permissions: dict[str, bool]) -> str:
    if not allowed(permissions, "create_folders"):
        return "I do not have permission to create folders yet."
...
def open_folder(permissions: dict[str, bool]) -> str:
    if not allowed(permissions, "open_apps"):
        return "I do not have permission to open folders yet."
    SAFE_FOLDER_ROOT.mkdir(parents=True, exist_ok=True)
    os.startfile(SAFE_FOLDER_ROOT)  # type: ignore[attr-defined]
    return f"Opening {SAFE_FOLDER_ROOT}."


def optional_ai_answer(prompt: str, permissions: dict[str, bool]) -> str:
    """Use cloud AI only when an API key exists. Never run AI output as commands."""
    if not allowed(permissions, "cloud_ai"):
        return "Cloud AI is disabled. I am staying local."
...
def handle_command(command: str, permissions: dict[str, bool]) -> tuple[str, bool]:
    ...
    if "open chrome" in command:
        return open_chrome(permissions), True
    if "open vscode" in command or "open vs code" in command:
        return open_vscode(permissions), True
    if "open youtube" in command:
        return open_youtube(permissions), True
    if command.startswith("search google for "):
        return search_google(command.replace("search google for ", "", 1), permissions), True
    if command.startswith("create folder "):
        return create_folder(command.replace("create folder ", "", 1), permissions), True
    if "open folders" in command or "open folder" in command:
        return open_folder(permissions), True
    ...
    if "what is bioinformatics" in command or "explain bioinformatics" in command:
        if not allowed(permissions, "bioinformatics_tools"):
            return "Offline bioinformatics tools are disabled.", True
    ...
    return optional_ai_answer(command, permissions), True

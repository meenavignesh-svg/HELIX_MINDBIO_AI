...
This generator is intentionally template-first so GitHub Actions can create new
projects without spending Codex/ChatGPT tokens. If OLLAMA_URL is provided, it can
ask a local Ollama server for small theme copy; otherwise it uses built-in text.
"""
...
def build_project(theme: dict) -> Path:
    ...
    A low-token generated chatbot project. The browser demo works immediately with built-in themed replies. Optional real AI mode works after Vercel deployment and asks each visitor for their own OpenAI API key.
    ...
    `tools/chatbot_factory/generate.py`
    """)
...
def main() -> None:
    ...
    print(f"Generated {folder.relative_to(REPO_ROOT).as_posix()}")

...
export default async function handler(req, res) {
  ...
  try {
    ...
    const input = messages
      ...
      .slice(-12)
      .map((message) => ({ role: message.role === 'assistant' ? 'assistant' : 'user', content: message.content.slice(0, 2000) }));

    if (input.length === 0) return res.status(400).json({ error: 'Send at least one message.' });

    const client = new OpenAI({ apiKey });
    const response = await client.responses.create({
      model: 'gpt-4.1-mini',
      instructions: 'You are Startup Coach, a practical AI assistant for startup ideas, MVP planning, customer discovery, and launch checklists.',
      input
    });

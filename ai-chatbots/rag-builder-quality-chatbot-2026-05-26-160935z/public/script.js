...
const quickActions = ["Summarize", "Risks", "Next steps"];
...
function demoReply(value) {
  const chosen = quickActions.find((item) => value.toLowerCase().includes(item.toLowerCase())) || 'Brief Builder';
  return `${chosen} mode: I can help with retrieval planning and source-grounded answers; practical quality check support; clear next-step guidance. Ask one specific question and I will turn it into a useful action. Requested focus: "${value}".`;
}
...
document.querySelectorAll('.chip').forEach((button) => {
  button.addEventListener('click', () => {
    ...
    text.focus();
    qualityEl.textContent = `Premium variant: mission-console / ${button.textContent}`;
  });
});
...
add("I can help with retrieval planning and source-grounded answers; practical quality check support; clear next-step guidance. Ask one specific question and I will turn it into a useful action.");

...
const quickActions = ["Checklist", "Blockers", "Automate"];
...
function demoReply(value) {
  const chosen = quickActions.find((item) => value.toLowerCase().includes(item.toLowerCase())) || 'Ops Check';
  return `${chosen} mode: I can help with safe wellness, appointment preparation, and care notes; practical daily brief support; clear next-step guidance. Ask one specific question and I will turn it into a useful action. Requested focus: "${value}".`;
}
...
document.querySelectorAll('.chip').forEach((button) => {
  button.addEventListener('click', () => {
    ...
    text.focus();
    qualityEl.textContent = `Premium variant: split-command / ${button.textContent}`;
  });
});
...
add("I can help with safe wellness, appointment preparation, and care notes; practical daily brief support; clear next-step guidance. Ask one specific question and I will turn it into a useful action.");
